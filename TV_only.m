function [u_k,info] = TV_only(image,u_solver,nScales,nAngles,alpha,lambda,maxIter,tol,sigma)
%TV only Convolution.  
%INPUTS:
...
%lambda - The augmented Lagrangian term.
...
while rel_res_error > tol && n_iter < maxIter
    ...
    info.prim_res(n_iter) = norm([- Dxu_k_1 + z1;...
                                  - Dyu_k_1 + z2;]);
    ...
    subplot(1,3,1); imagesc(b1); colorbar; title("[b1], Iteration = " + n_iter); drawnow;
    subplot(1,3,2); imagesc(b2); colorbar; title("[b2], Iteration = " + n_iter); drawnow;
    ...
    info.us(:,n_iter) = u_k(:);
    info.objective(n_iter) = 0.5*info.normL2DataFit(n_iter) + alpha*info.normL1z(n_iter);
...
title("Objective function"); legend('Objective(k)', '$\|f-u_k\|_2$', '$\alpha \|z_k\|_1$', 'Interpreter','Latex');
hold off
